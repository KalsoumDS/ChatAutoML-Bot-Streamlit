...
def plot_missing_values(analysis: Dict[str, Any]):
    """Crée un graphique des valeurs manquantes"""
    missing_data = []
    for col, info in analysis.get('missing_values', {}).items():
        if info['count'] > 0:
            missing_data.append({
                'Colonne': col,
...
def create_summary_table(analysis: Dict[str, Any]) -> pd.DataFrame:
    ...
    for col, info in analysis.get('columns', {}).items():
        summary_data.append({
            'Colonne': col,
            'Type': info['type'],
            'Type Python': info['dtype'],
            'Valeurs uniques': info['unique_count'],
...
def create_metrics_table(evaluation: Dict[str, Any], task_type: str) -> pd.DataFrame:
    """Crée un tableau des métriques détaillées"""
    train_metrics = evaluation.get('train_metrics', {})
    test_metrics = evaluation.get('test_metrics', {})
    ...
    for label, (train_key, test_key) in metrics.items():
        ...
        if train_val is not None or test_val is not None:
            results.append({
                ...
                'Écart': f"{abs(train_val - test_val):.4f}" if train_val is not None and test_val is not None else "N/A"
            })
...
def create_dataset_head_table(df: pd.DataFrame, n_rows: int = 10) -> pd.DataFrame:
    """Crée un tableau avec les premières lignes du dataset"""
...
def create_descriptive_stats_table(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for col in ['Mean', 'Std', 'Min', '25%', '50%', '75%', 'Max']:
        if col in stats.columns:
            stats[col] = stats[col].apply(lambda x: f"{x:.2f}" if pd.notna(x) else "N/A")
...
def create_preprocessing_summary_table(X_before: pd.DataFrame, X_after: np.ndarray, 
    ...
    for col in column_types.get('numeric', []):
        summary.append({
            'Colonne': col,
            'Type': 'Numérique',
            'Traitement': 'Imputation (médiane) + Standardisation',
            'Statut': '✓ Traité'
        })
    ...
    for col in column_types.get('categorical', []):
        summary.append({
            'Colonne': col,
            'Type': 'Catégorielle',
            'Traitement': 'Imputation (Unknown) + OneHotEncoding',
            'Statut': '✓ Traité'
        })
    
    # Informations sur les colonnes booléennes
    for col in column_types.get('boolean', []):
        summary.append({
            'Colonne': col,
            'Type': 'Booléenne',
            'Traitement': 'Imputation (mode) + Standardisation',
            'Statut': '✓ Traité'
        })
    ...
    summary.append({
        'Colonne': 'TOTAL',
        'Type': f'{X_before.shape[1]} colonnes',
        'Traitement': f'→ {n_features} features',
        'Statut': f'✓ {n_samples} échantillons'
    })
...
def create_train_test_split_table(X_train: np.ndarray, X_test: np.ndarray,
    ...
    split_data = [
        {
            'Dataset': 'Train',
            'Échantillons': X_train.shape[0],
            'Features': X_train.shape[1],
            'Proportion': f"{(X_train.shape[0] / (X_train.shape[0] + X_test.shape[0]) * 100):.1f}%"
        },
        {
            'Dataset': 'Test',
            'Échantillons': X_test.shape[0],
            'Features': X_test.shape[1],
            'Proportion': f"{(X_test.shape[0] / (X_train.shape[0] + X_test.shape[0]) * 100):.1f}%"
        },
        {
            'Dataset': 'TOTAL',
            'Échantillons': X_train.shape[0] + X_test.shape[0],
            'Features': X_train.shape[1],
            'Proportion': '100%'
        }
    ]
